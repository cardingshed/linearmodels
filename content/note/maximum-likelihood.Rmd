...
- $\mathcal{L}_{normal}(\theta) = \mathcal{L}(\mu,\sigma^2) = \prod_{i=1}^{n}\frac{1}{\sigma\sqrt{2\pi}}\exp\biggl[-\frac{1}{2}\biggl(\frac{x_i-\mu}{\sigma}\biggr)^2\biggr]$
...
In practice this will be implemented (as the default setting!) by your favourite statistics program 😎 however it is worth calling out that although the algebra looks scary at first, it's often relatively straightforward from the known density $f$.
A couple of noteworth points:
1) Normally we work with the *log likelihood*, $\textrm{log}(\mathcal{L}(\theta))$ because each $x_i$ likelihood is a very small number and adding small numbers rather than multiplying them helps avoid issues with numerical precision.  Note that a function $g$ and $\textrm{log}(g)$ have their maximum at the same point.
2) Maximising $g$ is the same as minimising $-g$ if that's easier for your program, and similarly $g$ and $2g$ have a maximum at the same point.
